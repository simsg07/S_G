...
public class BoomberBrain : MonsterAIBase
{
    ...
    private void TryRegisterBoomberContactHit(Collider other)
    {
        HitReceiver hitReceiver = FindHitReceiver(other.transform);
        if (hitReceiver == null ||
            !hitReceiver.CanAcceptHitSource(HitSourceType.BoomberContact) ||
            !contactHitReceivers.Add(hitReceiver))
        {
            return;
        }
        ...
        int contactDamage = monsterAttack != null ? monsterAttack.attackDamage : 1;
        DamageInfo damageInfo = new DamageInfo(
            ...
            HitSourceType.BoomberContact);

        hitReceiver.RegisterHit(damageInfo);
        LogDebug($"BoomberContact RegisterHit sent to {hitReceiver.name}");
    }
