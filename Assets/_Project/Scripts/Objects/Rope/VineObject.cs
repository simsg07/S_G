...
[RequireComponent(typeof(OpenPathOnBreak))]
[RequireComponent(typeof(Collider))]
public class VineObject : MonoBehaviour, IDamageable, ITriggerableObject
{
    ...
    [Tooltip("Vine is cut when this many valid hits are registered.")]
    [SerializeField] private int maxHitCount = 1;
    [Tooltip("When false, Vine ignores incoming hits.")]
    [SerializeField] private bool canBeTargeted = true;
    [Tooltip("Activate ConnectedObjectLink when Vine is cut.")]
    [SerializeField] private bool triggerConnectedObjectOnCut = true;
    [Tooltip("Delay before disabling the Vine GameObject after cut. Set 0 for immediate disable.")]
    ...
    [Header("Debug")]
    [Tooltip("Print Vine logs in the Console.")]
    ...
    [ContextMenu("Test Cut Vine")]
    private void TestCutVine()
    {
        CutVine();
    }

    [ContextMenu("Reset Vine")]
    private void ContextResetVine()
    {
        ResetVine();
    }
    ...
    private void CacheReferences(bool addMissingComponents)
    {
        ...
        openPathOnBreak = GetOrAddComponent(openPathOnBreak, addMissingComponents);
    ...
    private void ConnectHitReceiverEvents()
    {
        ...
        hitReceiver.onMaxHit.RemoveListener(CutVine);
        hitReceiver.onMaxHit.AddListener(CutVine);
    }
    ...
    private void Log(string message)
    {
        if (debugMode)
        {
            Debug.Log($"[VineObject] {message}", this);
        }
    }
    ...
    private void LogWarning(string message)
    {
        if (debugMode)
        {
            Debug.LogWarning($"[VineObject] {message}", this);
        }
    }
}
