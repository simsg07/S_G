...
public class MonsterDetection : MonoBehaviour
{
        ...
        out Collider blockingCollider)
    {
        ...
        return !requireLineOfSight || HasLineOfSight(observer, target, out blockingCollider);
    }
    ...
    public bool HasLineOfSight(Transform observer, Transform target, out Collider blockingCollider)
    {
        ...
        if (obstacleLayerMask.value == 0)
        {
            if (debugMode && Time.time >= nextLineOfSightLogTime)
            {
                ...
                Debug.LogWarning("[MonsterDetection] LOS check failed because obstacleLayerMask is empty.", this);
            }
        ...
        for (int i = 0; i < hits.Length; i++)
        {
            ...
            bool layerBlocks = (obstacleLayerMask.value & (1 << candidate.gameObject.layer)) != 0;
