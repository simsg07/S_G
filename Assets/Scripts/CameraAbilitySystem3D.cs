...
{
    [Header("Unlocked")]
    [SerializeField] private CameraAbilityFlags unlockedAbilities = CameraAbilityFlags.Shutter | CameraAbilityFlags.Focus | CameraAbilityFlags.Flash; // 처음부터 사용할 수 있는 카메라 기능 목록입니다.
    ...
    [SerializeField] private Key directWorldSwitchKey = Key.Q; // 카메라 모드가 아닐 때 바로 월드를 변경하는 키입니다.
    [SerializeField] private Key previousCameraSlotKey = Key.Q; // 카메라 기능 슬롯을 왼쪽으로 이동하는 키입니다.
    ...
    [SerializeField] private float flashRadius = 4f; // 라이트를 켤 때 반응 오브젝트를 감지하는 반경입니다.
    ...
    [SerializeField] private float flashCooldown = 0.55f; // 라이트 기능 재사용 대기 시간입니다.
    [SerializeField] private float relayCooldown = 0.35f; // 릴레이 기능 재사용 대기 시간입니다.

    [Header("Flash Light")]
    [SerializeField] private float flashLightDuration = 0.16f; // 순간 플래시 잔광이 사라지는 시간입니다.
    ...
    private readonly HashSet<IFlashReactive3D> flashTargets = new HashSet<IFlashReactive3D>();
    ...
    public bool BlocksLegacyAttackInput => enabled && cameraModeActive && usePrimaryFireForCameraAbility;
    public bool BlocksLegacyDashInput => enabled && useSecondaryFireForCameraMode;
    ...
    private void Update()
    {
        ...
        UpdateCameraFrame();
        if (!cameraModeActive && WasPressed(keyboard, directWorldSwitchKey))
        {
            TryUseFocus();
            return;
        }
    ...
    {
        CameraAbilityId[] slots = { CameraAbilityId.Shutter, CameraAbilityId.Focus, CameraAbilityId.Flash };
        int currentIndex = 0;
    ...
    {
        return (unlockedAbilities & ToFlag(ability)) != 0;
    }
    ...
    private void TryUseShutter()
    {
        ...
        if (IsMarked(targetComponent))
        {
            if (TryRelayMarkedTarget(targetComponent))
            {
                StartRemarkCooldown(targetComponent);
            }

            return;
        }

        if (IsInRemarkCooldown(targetComponent))
        {
            return;
        }
    ...
    private void TryUseFlash()
    {
        if (!IsUnlocked(CameraAbilityId.Flash) || flashCooldownTimer > 0f)
        {
            return;
        }

        Vector3 flashOrigin = useMouseFrameTargeting ? GetMouseWorldPoint(transform.position.z) : transform.position;
        cameraLightOn = !cameraLightOn;
        SetCameraLight(cameraLightOn, flashOrigin);

        if (cameraLightOn)
        {
            flashTargets.Clear();
            int hitCount = Physics.OverlapSphereNonAlloc(flashOrigin, flashRadius, targetHits, targetMask, QueryTriggerInteraction.Collide);
            for (int i = 0; i < hitCount; i++)
            {
                IFlashReactive3D reactive = ResolveInterface<IFlashReactive3D>(targetHits[i]);
                if (reactive == null || flashTargets.Contains(reactive))
                {
                    continue;
                }

                flashTargets.Add(reactive);
                reactive.OnCameraFlash(this);
            }
        }
    ...
    {
        GameObject lightObject = new GameObject("Camera Flash Light", typeof(Light));
    ...
    private void TriggerFlashLight(Vector3 origin)
    {
        if (flashLight == null)
        {
            SetupFlashLight();
        ...
        flashLight.transform.position = origin + new Vector3(0f, 0f, -0.55f);
        flashLight.range = flashLightRange;
        flashLight.color = flashLightColor;
        flashLightTimer = Mathf.Max(0.01f, flashLightDuration);
    ...
    private void UpdateFlashLight()
    {
        ...
        float normalized = Mathf.Clamp01(flashLightTimer / Mathf.Max(0.01f, flashLightDuration));
        flashLight.intensity = flashLightIntensity * normalized * normalized;
    ...
    private void TickCooldowns()
    {
        ...
        flashCooldownTimer = Mathf.Max(0f, flashCooldownTimer - cooldownDelta);
