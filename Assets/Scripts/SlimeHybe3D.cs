...
public class SlimeHybe3D : MonoBehaviour, IAttackable3D
{
    ...
    private void SpawnBalloonSlimes()
    {
        for (int i = 0; i < balloonSpawnCount; i++)
        {
            ...
            Vector3 spawnPosition = transform.position + new Vector3(xOffset, bodySize.y * 0.9f + 0.5f, 0f);
    ...
    private void ScatterMucusProjectiles()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            projectile.transform.position = transform.position + direction * (bodySize.x * 0.5f + 0.18f);
