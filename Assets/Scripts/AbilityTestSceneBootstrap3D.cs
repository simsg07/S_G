...
public class AbilityTestSceneBootstrap3D : MonoBehaviour
{
    ...
    private PlayerHealth3D playerHealth;
    ...
    private void CreatePlayer()
    {
        ...
        playerHealth = player.AddComponent<PlayerHealth3D>();
    ...
    private void CreateAbilityCourse()
    {
        ...
        GameObject worldAPlatform = CreateBox("World A Platform", new Vector3(-0.8f, 1.2f, 0f), new Vector3(2.6f, 0.32f, 1f), worldAColor);
        AddWorldVariant(worldAPlatform, ResearchWorldId.WorldA, false);

        GameObject worldBPlatform = CreateBox("World B Platform", new Vector3(-0.8f, 2.35f, 0f), new Vector3(2.6f, 0.32f, 1f), worldBColor);
        AddWorldVariant(worldBPlatform, ResearchWorldId.WorldB, false);
        ...
        AddWorldVariant(relayStone, ResearchWorldId.WorldA, false);
        relayStone.AddComponent<RelayTransferable3D>();

        GameObject worldBMarker = CreateBox("World B Marker", new Vector3(4.35f, 1.45f, 0f), new Vector3(0.9f, 0.2f, 0.8f), worldBColor);
        AddWorldVariant(worldBMarker, ResearchWorldId.WorldB, false);
        ...
        AddWorldVariant(goal, ResearchWorldId.WorldA, true);
    }
    ...
    private void RefreshHud()
    {
        ...
        if (helpText != null)
        {
            int currentHealth = playerHealth != null ? playerHealth.CurrentHealth : 1;
            helpText.text =
                $"Health {currentHealth}/1\n" +
                "Mouse : Aim camera frame\n" +
                "Left Click / E : Shutter Mark target\n" +
                "Marked target + Shutter : Relay\n" +
                "Q : Switch World A/B\n" +
                "F : Flash at mouse frame\n" +
                "R : Relay stone to other world";
        }
    }
    ...
    private WorldVariant3D AddWorldVariant(GameObject target, ResearchWorldId world, bool existsInBothWorlds)
    {
        WorldVariant3D variant = target.AddComponent<WorldVariant3D>();
        variant.ExistsInBothWorlds = existsInBothWorlds;
        if (!existsInBothWorlds)
        {
            variant.SetActiveWorld(world);
        }
        else
        {
            variant.Refresh(WorldSystem3D.ActiveWorld);
        }
