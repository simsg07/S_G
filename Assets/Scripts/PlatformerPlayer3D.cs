...
public class PlatformerPlayer3D : MonoBehaviour
{
    ...
    [SerializeField] private float dashSpeed = 14f; // 대쉬 이동 속도입니다.
    ...
    private void ReadMovementInput()
    {
        ...
        if (mouse != null && mouse.rightButton.wasPressedThisFrame && !IsSecondaryFireReservedForCamera())
        {
            TryStartDash();
        }
    }

    private bool IsSecondaryFireReservedForCamera()
    {
        ...
        return cameraAbilities != null && cameraAbilities.BlocksLegacyDashInput;
    }
    ...
    {
        if (!isGrounded || IsKnockedBack || IsDashing || robotLegRetracting)
        {
            return;
        }
