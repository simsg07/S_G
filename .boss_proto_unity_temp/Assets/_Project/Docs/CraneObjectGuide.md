...
## 프리팹 값이 Play 때 되돌아갈 때

1. `CraneMovingVisualAligner3D.alignVisualsOnStart`가 false인지 확인한다.
2. `alignVisualsOnValidate`가 false인지 확인한다.
3. `preserveManualVisualOffsets`가 true인지 확인한다.
4. `CraneObject.applyColliderOnStart`와 `applyColliderOnValidate`가 false인지 확인한다.
5. `preserveManualColliderSettings`가 true인지 확인한다.
6. `DataDrivenObjectController.applyMode`가 `ManualOnly`, `allowRuntimeOverwrite`가 false인지 확인한다.

Visual 정렬이 필요할 때만 `Align Visuals Once`, 케이블만 갱신할 때는 `Refresh Cable Only`를 실행한다. Collider 템플릿 값을 의도적으로 적용할 때만 `Apply Collider Setup Once`를 실행한다. 프리팹에서 직접 조정한 Transform, SpriteRenderer, BoxCollider 값이 기본 기준이다.

## Horizontal Crane

기존 `Crane_Body`는 `moveAxis = Horizontal`이며 Point_A와 Point_B의 X축 구간을 왕복한다. 기존 `cabinYOffset`과 Lever 연결 방식을 그대로 사용한다.

## Vertical Crane

`Crane_Vertical.prefab`은 `moveAxis = Vertical`이며 `Crane_RailPath_Vertical.prefab`의 같은 X 좌표를 가진 Point_A와 Point_B 사이를 Y축으로 왕복한다. 씬에서 `CraneObject.railPath`에 Vertical RailPath를 연결하고, 기존 `Lever` 프리팹의 `targetCrane`에 Vertical Crane의 `CraneObject`를 연결한다. `Reset To Point A/B`, `Test Move To A/B`, Lever의 `Test Interact`로 확인한다.
# 기능 테스트 모드
