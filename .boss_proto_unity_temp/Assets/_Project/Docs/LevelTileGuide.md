...
프리팹 방식으로 맵을 만들 때는 Root Transform을 정수 좌표 격자에 둡니다. 예를 들어 `Tile_Visual`, `Floor_Tile`, `Wall_Tile`, `Block_Tile`을 `(3, 2, 0)`에 두면 실제 보이는 1칸 블럭은 `(3,2)`에서 시작해 오른쪽/위쪽으로 한 칸을 채웁니다.

- Root Position: 칸의 왼쪽 아래/시작 좌표
- Visual child Position: `(0.5, 0.5, 0)`
- 1x1 Collider Center: `(0.5, 0.5, 0)`
- `Floor_Collision` Center X: `width * 0.5`
- Root Scale은 항상 `(1, 1, 1)` 유지

이 기준을 쓰면 Scene View의 네모칸 경계와 프리팹 경계가 맞고, 좌표를 찍어서 설치하는 느낌으로 배치할 수 있습니다.
...
Tile Palette는 이미지 색칠 전용입니다. 충돌과 감지 차단은 Tilemap이 아니라 아래 프리팹으로 배치합니다.

- `Floor_Collision`: Player가 실제로 밟는 바닥
- `Wall_Tile` / `Block_Tile`: 이동 차단, 벽 충돌, Monster 시야 차단
- DetectionBlock 계열 프리팹: 보이지 않는 Monster 인식 차단
- LightSightBlock 계열 프리팹: Light 인식 차단

`Tilemap_GroundVisual`에 칠한 이미지만으로는 Player가 밟을 수 없습니다. 반드시 같은 구간에 `Floor_Collision`을 배치합니다.
